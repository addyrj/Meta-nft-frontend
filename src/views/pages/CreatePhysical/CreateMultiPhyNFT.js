...
import {
  ...
  uploadPhysicalContractHandler,
  getPhysicalTokenId,
  createPhysicalNFTBlockchainHanlder,
  addImageHandler,
  placePhyMultiOrderAPIHandler,
...
import BrandMultiCollection from "../PhysicalNFT/BrandMultiCollection";
...
export default function ResellNFT(props) {
  ...
  const brandCollectionType = "MULTI_BRAND";
    ...
    if (
      ...
      if (formValue.collectionIMG !== "") {
        await addImageHandler(formValue.collectionIMG)
          .then(async (res) => {
            let receipt = "";
            await uploadPhysicalContractHandler(
              formValue.displayName,
              formValue.symbol,
              res,
              formValue.collectionIMG,
            ...
            console.log("multiaddress", receipt);
            if (receipt != false) {
  ...
  //             return data?.displayName.trim() === "Brand_Multiple";
  //           });
  ...
  const getBrandListHanlder = async (id) => {
    ...
    try {
      ...
      axios({
        method: "GET",
        url: ApiConfig.getCollectionOnBrandMultiple,
        headers: {
          token: accessToken,
        },
        params: {
        ...
        if (res.data.statusCode === 200) {
          // const filterData5 = res.data.result.docs.filter((data) => {
          //   return data?.contractAddress === multiNftTokenAddress;
          // });
    ...
    if (chainId === ACTIVE_NETWORK) {
      if (
        ...
        if (copies > 1) {
            ...
            .then(async (res) => {
              ...
              if (receipt) {
                ...
                try {
                  console.log("+++++++++++++++++1++++++++++++++");
                  if (
                    await createPhysicalNFTBlockchainHanlder(
                    ...
                    const tokenId = await getPhysicalTokenId(
                      selectedbrandCollection?.contractAddress,
                      MultiPhyNftTokenABI,
                      library,
                      account
          ...
          toast.warn("Please enter more then 1 copies.");
        }
      } else {
              ...
              try {
                ...
                if (receipt) {
                  ...
                  if (creRes && creRes.data.statusCode === 200) {
                    setIsLoading(true);
                    setTransactionStatus("Create Item");
                    const placeres = await placePhyMultiOrderAPIHandler(
                      createBody,
      ...
      <Box className={classes.bannerBox}>
        <Container maxWidth="md">
          <Box className={classes.root}>
            ...
            <Box>
              <Typography>Multiple edition on QIE</Typography>
            </Box>
            <Box>
                        ...
                        <Box mt={2}>
                          ...
                          <FormControl fullWidth>
                            <TextField
                              ...
                              error={isSubmit1 && symbole === ""}
                              helperText={
                                isSubmit1 &&
                                symbole === "" &&
                                "Please enter symbole"
                              }
                              placeholder="e. g. 'Redeemable T-Shirt with logo'"
                            />
                          </FormControl>
