...
import {
  ...
  placePhysicalSingleOrderBlockchainHandler,
  placePhysicalOrderBlockchainHandler,
} from "src/services";
import {
  NetworkDetails,
  marketplaceContract,
    ...
    if (price !== "" && parseFloat(price) > 0) {
      try {
        ...
        if (
          ...
          if (
            await placePhysicalSingleOrderBlockchainHandler(
              orderDetails?.collectionId?.brandId
                ? orderDetails?.collectionId?.brandId
                : orderDetails?.brandId?._id,
              singlemarketplaceContract,
              SingPhyMarketABI,
              library,
              account,
            ...
            const placeres = await axios({
              method: "post",
              url: Apiconfig["createPhysicalOrder"],
              data: body,
              headers: {
                token,
              },
          ...
          <Grid container spacing={4}>
            <Grid item xs={12} md={6}>
              ...
              <Box className={classes.boxsection}>
                ...
                {/* <Box>{orderDetails?.tokenName}</Box> */}
                {console.log(
                  "barQRcodeLink",
                  orderDetails?.nftId?.barQRcodeLink
                )}
                <Box className={classes.box3}>
                  ...
                  orderDetails?.physicalType === "SINGLE" ? (
                    <>
                      ...
                      >
                        {orderDetails?.nftId?.barQRcodeLink !== undefined && (
                          <>
                            <Box>
                              <QRCodeSVG
